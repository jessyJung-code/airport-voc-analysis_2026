# 보안검색 서비스 VOC · 출국장 여객흐름 통합 대시보드

Streamlit 기반 2페이지 대시보드
...
`requirements.txt`, `runtime.txt`는 이 대화에서 만든 파일을 그대로 커밋하면 됩니다.
**`voc_raw.xlsx`와 `xovis_flow.csv`는 원본 데이터라 별도로 저장소에 추가**해야 합니다.
...
5. 배포 후 데이터 파일을 못 찾는다는 에러가 뜨면, 저장소에 데이터 파일이
   실제로 커밋됐는지, 파일명이 `voc_raw.xlsx` / `xovis_flow.csv`와 정확히
   일치하는지 확인하세요 (대소문자 포함)
...
같은 폴더에 `voc_raw.xlsx`, `xovis_flow.csv`를 두고 실행하면 됩니다.
